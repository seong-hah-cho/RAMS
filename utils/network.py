...
def RFAB(x, filters, kernel_size, r):
    ...
    print("After first conv3d_weightnorm in RFAB:", x.shape)
    x = layers.ReLU()(x)
    x = conv3d_weightnorm(filters, kernel_size)(x)
    print("After second conv3d_weightnorm in RFAB:", x.shape)
...
def RTAB(x, filters, kernel_size, r):
    ...
    print("After first conv2d_weightnorm in RTAB:", x.shape)
    x = layers.ReLU()(x)
    x = conv2d_weightnorm(filters, kernel_size)(x)
    print("After second conv2d_weightnorm in RTAB:", x.shape)
...
def RAMS(scale, filters, kernel_size, channels, r, N):
    ...
    print("After conv3d_weightnorm for upscaling in RAMS:", x.shape)
    x = layers.Lambda(lambda x: x[...,0,:])(x)
    print("After selecting channel in RAMS:", x.shape)
    x = layers.Lambda(lambda x: tf.nn.depth_to_space(x, scale))(x)
    print("After depth_to_space in RAMS:", x.shape)
